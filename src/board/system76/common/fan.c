...
void fan_update_target(void) {
    ...
    if (fan_max) {
        ...
        if ((fan1_level == 0) && ((sys_temp - FAN_TEMP_HYSTERESIS) < FAN1.points[0].temp)) {
    ...
    if (fan_max) {
        ...
        if ((fan2_level == 0) && ((sys_temp - FAN_TEMP_HYSTERESIS) < FAN2.points[0].temp)) {
            // If temp is below point 0 with hysteresis, then turn off.
            fan2_pwm_target = 0;
        } else {
